...
bool CGG::Init(int windowWidth, int windowHeight )
{
	if( SDL_Init( SDL_INIT_VIDEO ) < 0 )
	{
		// Something went very wrong in initialisation, all we can do is exit
		std::cout<<"CGG Framework: Whoops! Something went very wrong, cannot initialise SDL :("<<std::endl;
		return false;
	}
	...
	int winPosX = 100;
	int winPosY = 100;
	_window = SDL_CreateWindow("CGG Framework",  // The first parameter is the window title
		winPosX, winPosY,
	...
	{
		// Something went very wrong in initialisation, all we can do is exit
		std::cout<<"CGG Framework: Whoops! Something went very wrong, cannot create SDL window :("<<std::endl;
		return false;
	}
	...
	{
		// Something went very wrong in initialisation, all we can do is exit
		std::cout<<"CGG Framework: Whoops! Something went very wrong, cannot obtain SDL rendering context :("<<std::endl;
		return false;
	}
...
void CGG::SetBackground( int red, int green, int blue )
{
